...
const updateCourse = async (
	...
	// Validate inputs
	id = validation.validateString('id', id);
	if (!ObjectId.isValid(id)) throw `Error: ${id} is not a valid ID.`;
	...
	// Check for duplicate courseId (ignore current course)
	const exists = await coursesCollection.findOne({ courseId, _id: { $ne: new ObjectId(id) } });
	if (exists) throw `Error: courseId ${courseId} is already in use by another course.`;
	...
	if (!updatedInfo) throw `Error: could not update course with ID ${id}.`;
